...
ヘッダーに「一覧」と「マーク」の2つのタブがある。一覧タブでスクラップの一覧、マークタブでマーク済みエントリの一覧を切り替えられる。

### スクラップの作成

一覧画面の「+ 新規作成」ボタンからタイトルとタグを入力して作成する。タグはカンマ区切りで複数指定できる。
...
- Markdown / Preview タブで編集とプレビューを切り替え（`Cmd/Ctrl + E` でも切替可能）
...
エントリをブックマークして、あとから素早くアクセスできる。

- エントリヘッダーのブックマークアイコンをクリックしてマーク/解除
- 詳細画面ヘッダーのブックマークフィルターボタンで、マーク済みエントリのみに絞り込み
- マーク絞り込み中は「まとめてコピー」ボタンで全マーク済みエントリを一括コピー

### マーク一覧

ヘッダーの「マーク」タブで、全スクラップ横断のマーク済みエントリ一覧を表示する。

- クリックで該当エントリにジャンプ（ハイライト付き）
- `Cmd/Ctrl + Click` で複数選択 → 「選択中をまとめてコピー」
- 各エントリのマーク解除・コピーボタン
- 検索バーで絞り込み

### 埋め込み記法

エディタの「+ 埋め込み」ボタンから挿入するか、直接記法を書く。URLをペーストすると自動で埋め込み記法に変換される（設定でオフ可）。
...
スクラップには3つの状態がある。詳細画面のステータスラベルをクリックして切り替える。

| ステータス | 説明 |
| --- | --- |
| Open | 作業中。作成時のデフォルト |
| Closed | 完了・終了。クローズ済みスクラップにはクローズ日のバナーが表示される |
| Archived | 非表示。一覧のフィルターを「Archived」にすると表示される |
...
- 各スクラップのメニューからピン留め/解除・アーカイブ・削除（確認ダイアログあり）

### 詳細画面の操作

- タイトルクリックで直接編集
- タグの表示・インライン編集（タグ行の編集アイコン、またはタグがない場合は「+ タグを追加」から）
- ステータスラベルクリックでステータス切り替え
- アクションメニュー（…）からJSONコピー、ファイルを開く、削除
- 各エントリの編集・削除（確認ダイアログあり）・ドラッグ&ドロップ並べ替え
- アウトラインボタンからエントリ一覧を開き、クリックでハイライト付きジャンプ。ホバーでツールチップ表示
- 全幅表示の切り替えボタン
- ヘッダー右側の ? ボタンからショートカット・操作ガイドを表示
...
| URLの自動埋め込み | ON | URLをペーストしたとき自動で埋め込み記法に変換する |
